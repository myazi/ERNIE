...
CHECKPOINT_PATH=./data/ema_step_1365_infer  ## 1) 进行infer时，模型不带infer后缀，2) 使用带后缀的infer模型进行服务部署
DATA_PATH=./data/
MODEL_PATH=./ernie/config/base

echo `pwd`
cat run_paddle_service.sh >${CHECKPOINT_PATH}.log

#export CPU_NUM=8
#/root/anaconda3/envs/paddle_cpu/bin/python -u ernie_server/paddle_service.py \
/root/anaconda3/envs/paddle_116/bin/python -u ernie/paddle_service.py \
                   --use_cuda true \
                   --batch_size 16 \
                   ...
                   --server_port 8081 > ${CHECKPOINT_PATH}.log 2>&1 &
