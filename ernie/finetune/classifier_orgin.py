...
def kl_for_probs(p, q):
    log_p = fluid.layers.log(p)
    log_q = fluid.layers.log(q)
    neg_ent = fluid.layers.reduce_sum(p * log_p, dim=-1)
    neg_cross_ent = fluid.layers.reduce_sum(p * log_q, dim=-1)
...
def create_model(args,
    ...
    def _model(src_ids, sent_ids, pos_ids, task_ids, input_mask, labels,
     qids, is_noise=False):
    ...
    if not is_prediction:
        graph_vars = _model(src_ids, sent_ids, pos_ids, task_ids, input_mask, labels,
                            qids)
    ...
    while True:
        try:
            ...
            if is_classify:
                np_preds = np.argmax(np_probs, axis=1).astype(np.float32)
                preds.extend(np_preds)
                batch_score = np_probs[:,1:2].reshape(-1).tolist()##获取二分类最后一列，即为label=1的概率
