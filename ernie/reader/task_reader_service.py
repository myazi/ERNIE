...
class BaseReader(object):
    ...
    def get_pre_examples(self, data):
        ...
        for (i, line) in enumerate(data):
            text_a = (line[0].replace(' ', '').replace('\t', '').replace('\r', ''))
            text_b = (line[1].replace(' ', '').replace('\t', '').replace('\r', ''))
            text_c = (line[2].replace(' ', '').replace('\t', '').replace('\r', ''))
            label = '0'
            pre_data =[text_a, text_b, text_c, label]
            #print(pre_data)
