...
#import tokenization
import tokenizer_char_word_mix as tokenization # shenyan ernie 2.0 - base tokenizer
...
#                example = Example(idx, query, pos2[:pos2_num], pos1[:pos1_num], neg[:neg_num])
                example = Example(idx, query, pos, neg)
                examples.append(example)
...
#                self.all_qp_count += pos1_num + pos2_num + neg_num
    ...
    def _convert_example_to_record(self, example, max_seq_length, tokenizer):
        ...
        np.random.shuffle(example.pos)
        np.random.shuffle(example.neg)
        pos_idx, neg_idx = 0, 0

        while pos_idx < len(example.pos) or neg_idx < len(example.neg):
            if pos_idx < len(example.pos):
                title, para, score = example.pos[pos_idx]
                pos_idx += 1
                record = self._qp_to_record(qid, query, title, para, int(score), max_seq_length, tokenizer)
                records.append(record)
            if neg_idx < len(example.neg):
                title, para, score = example.neg[neg_idx]
                neg_idx += 1
                record = self._qp_to_record(qid, query, title, para, int(score), max_seq_length, tokenizer)
                records.append(record)
