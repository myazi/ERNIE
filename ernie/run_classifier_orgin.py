...
import reader.task_reader_orgin as task_reader
from model.ernie import ErnieConfig
from finetune.classifier_orgin import create_model, evaluate, predict
...
def main(args):
    ...
    if args.use_cuda:
        ...
        dev_count = 8 #int(os.environ.get('CPU_NUM', multiprocessing.cpu_count()))
    ...
    #if args.do_train and not args.is_regression:
    #    reader_class = task_reader.ClassifyReaderPairwise
    #else:
    reader_class = task_reader.ClassifyReader
            ...
            try:
                ...
                if steps % args.skip_steps != 0:
                    ...
                    if args.is_classify:
                        ...
                        total_acc.append(outputs["accuracy"])
                        total_acc = total_acc[-20:]
                        log.info(
                            "epoch: %d, progress: %d/%d, step: %d, ave loss: %f, "
                            "ave acc: %f, speed: %f steps/s" %
                            (current_epoch, current_example, num_train_examples,
                             steps,
                             1.0 * sum(total_loss) / len(total_loss),
                             1.0 * sum(total_acc) / len(total_acc),
                ...
                if nccl2_trainer_id == 0:
                    if steps % args.save_steps == 0:
                        # if args.use_ema:
                            ...
                            #                                 "ema_step_" + str(steps))
                            #         log.info('save ema model to %s' % save_path)
                    ...
                    if steps % args.validation_steps == 0 or last_epoch != current_epoch:
                        ...
                        if args.do_test:
                            #if args.use_ema:
                            #    log.info('Predict with EMA parameters')
                            #    with ema.apply(exe, need_restore=False):
    ...
    if args.do_test:
        if args.use_ema:
            print('Predict with EMA parameters')
            try:
                with ema.apply(exe, need_restore=False):
                    ...
                    log.info('save_ema_model to %s' % save_path)
                    fluid.io.save_persistables(exe, save_path, test_prog)
            except:
                print('Not suppport for `need_restore=False`')
                with ema.apply(exe):
                    predict_wrapper(args, reader, exe, test_prog, test_pyreader, graph_vars,
                                    0, 0)
        else:
    ...
    # final eval on dianostic, hack for glue公开测试集
    if args.diagnostic:
